...
export default function AdminProductCard({
  ...
  return (
    ...
    <Card className="mt-2 border border-zinc-600 rounded-lg p-6 hover:shadow-md transition-shadow bg-primary-foreground/30">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
        <div className="space-y-4 min-w-0 flex-1">
          <CardTitle className="text-xl font-semibold flex justify-between items-center">
            ...
            <Badge
              className={cn(
                product.status === "pending" && "bg-yellow-400 text-black",
                product.status === "approved" && "bg-green-500 text-black",
                product.status === "rejected" && "bg-red-500 text-black",
              )}
            >
              {product.status}
            </Badge>
        ...
        <CardFooter>
          <Button
            variant={"outline"}
            className="hover:bg-red-400 hover:cursor-pointer border border-slate-800"
          >
            <TrashIcon className="size-3 border border-slate-800" />
            Delete
          </Button>
        </CardFooter>
